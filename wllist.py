...
from pyinterop.protocol.ext_workspace_v1 import ExtWorkspaceManagerV1
from pyinterop.protocol.ext_workspace_v1 import ExtWorkspaceGroupHandleV1
from pyinterop.protocol.ext_workspace_v1 import ExtWorkspaceHandleV1

from pyinterop.protocol.ext_foreign_toplevel_workspace_unstable_v1 import ExtForeignToplevelWorkspaceHandleV1
from pyinterop.protocol.ext_foreign_toplevel_workspace_unstable_v1 import ExtForeignToplevelWorkspaceManager
...
def add_workspace_info(
    ...
    def handle_workspace_group(_, workspace_group_handle: ExtWorkspaceGroupHandleV1):
        print(f"Handling Workspace Group: {workspace_group_handle}")

    workmgr = registry.bind(id_num, ExtWorkspaceManagerV1, version)
    workmgr.dispatcher["workspace_group"] = handle_workspace_group
    #def handle_format(_, fmt: int) -> None:
    #    format_info = next(f for f in WlShm.format if f.value == fmt)
    #    append(f"{hex(format_info.value)}: {format_info.name}")
...
def handle_registry_global(
    ...
    if interface in ("wl_seat", "wl_output", "wl_shm"):
        ...
        info.roundtrip_needed = True
        add_workspace_info(info, registry, id_num, interface, version)
